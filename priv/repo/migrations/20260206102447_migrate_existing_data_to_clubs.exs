defmodule Sideout.Repo.Migrations.MigrateExistingDataToClubs do
  ...
  def up do
    ...
    result =
      repo().query!("SELECT id FROM clubs WHERE name = 'Default Club' ORDER BY id DESC LIMIT 1")
    ...
    repo().query!(
      "UPDATE session_templates SET club_id = #{default_club_id} WHERE club_id IS NULL"
    )
    ...
    alter table(:sessions) do
      modify :club_id, references(:clubs, on_delete: :delete_all),
        null: false,
        from: references(:clubs, on_delete: :delete_all)
    end
